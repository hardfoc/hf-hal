...
bool EncoderManager::Deinitialize() noexcept {
    ...
    if (interrupt_enabled_) {
        DisableInterrupt(ONBOARD_ENCODER_INDEX);
    }
...
As5047uError EncoderManager::ReadAngle(uint8_t deviceIndex, uint16_t& angle) noexcept {
    ...
    return handler->ReadAngle(angle);
}
...
As5047uError EncoderManager::ReadVelocityRPM(uint8_t deviceIndex, double& velocity_rpm) noexcept {
    ...
    return handler->ReadVelocityRPM(velocity_rpm);
}
...
bool EncoderManager::Initialize() noexcept {
    ...
    // Initialize interrupt GPIO (optional)
    InitializeInterruptGpio(); // Best effort, don't fail if this doesn't work
...
bool EncoderManager::InitializeInterruptGpio() noexcept {
    // Create semaphore for WaitForInterrupt()
    interrupt_semaphore_ = xSemaphoreCreateBinary();
    if (!interrupt_semaphore_) {
        Logger::GetInstance().Warning("EncoderManager", "Failed to create interrupt semaphore");
        return false;
    }
    
    // Note: Actual GPIO interrupt setup would depend on board design
    // For now, just indicate that interrupt support is available but not configured
    Logger::GetInstance().Info("EncoderManager", "Interrupt support initialized (GPIO configuration required)");
    return true;
}
...
bool EncoderManager::ConfigureInterrupt(uint8_t deviceIndex, std::function<void()> callback) noexcept {
    MutexLockGuard lock(manager_mutex_);
    
    if (deviceIndex >= MAX_ENCODER_DEVICES || !device_active_[deviceIndex]) {
        Logger::GetInstance().Error("EncoderManager", "Invalid or inactive device index %u for interrupt configuration", deviceIndex);
        return false;
    }
    ...
    // TODO: Implement actual GPIO interrupt configuration based on board design
    // This would involve:
    // 1. Getting the appropriate GPIO pin for the encoder's interrupt/data ready signal
    // 2. Configuring it as input with appropriate pull-up/down
    // 3. Setting up interrupt on rising/falling edge
    // 4. Registering the interrupt handler
    
    interrupt_configured_ = true;
    Logger::GetInstance().Info("EncoderManager", "Interrupt configured for device %u (implementation pending)", deviceIndex);
    
    return true;
}

bool EncoderManager::EnableInterrupt(uint8_t deviceIndex) noexcept {
    MutexLockGuard lock(manager_mutex_);
    
    if (!interrupt_configured_) {
        Logger::GetInstance().Error("EncoderManager", "Interrupt not configured - call ConfigureInterrupt() first");
        return false;
    }
    
    // TODO: Enable the actual GPIO interrupt
    interrupt_enabled_ = true;
    Logger::GetInstance().Info("EncoderManager", "Interrupt enabled for device %u", deviceIndex);
    
    return true;
}

bool EncoderManager::DisableInterrupt(uint8_t deviceIndex) noexcept {
    MutexLockGuard lock(manager_mutex_);
    
    // TODO: Disable the actual GPIO interrupt
    interrupt_enabled_ = false;
    Logger::GetInstance().Info("EncoderManager", "Interrupt disabled for device %u", deviceIndex);
    
    return true;
}

bool EncoderManager::IsInterruptEnabled(uint8_t deviceIndex) const noexcept {
    MutexLockGuard lock(manager_mutex_);
    return interrupt_configured_ && interrupt_enabled_;
}

bool EncoderManager::WaitForInterrupt(uint8_t deviceIndex, uint32_t timeout_ms) noexcept {
    if (!interrupt_semaphore_) {
        return false;
    }
    
    TickType_t ticks_to_wait = (timeout_ms == 0) ? portMAX_DELAY : pdMS_TO_TICKS(timeout_ms);
    BaseType_t result = xSemaphoreTake(static_cast<SemaphoreHandle_t>(interrupt_semaphore_), ticks_to_wait);
    
    return (result == pdTRUE);
}

uint32_t EncoderManager::GetInterruptCount(uint8_t deviceIndex) const noexcept {
    return interrupt_count_.load(std::memory_order_acquire);
}

void EncoderManager::GpioInterruptHandler(BaseGpio* gpio, hf_gpio_interrupt_trigger_t trigger, void* user_data) noexcept {
    EncoderManager* manager = static_cast<EncoderManager*>(user_data);
    if (!manager) return;
    
    // Increment interrupt counter
    manager->interrupt_count_.fetch_add(1, std::memory_order_acq_rel);
    
    // Give semaphore to wake up any waiting tasks
    if (manager->interrupt_semaphore_) {
        BaseType_t higher_priority_task_woken = pdFALSE;
        xSemaphoreGiveFromISR(static_cast<SemaphoreHandle_t>(manager->interrupt_semaphore_), &higher_priority_task_woken);
    ...
    if (manager->interrupt_callback_) {
        manager->interrupt_callback_();
    }
}
...
void EncoderManager::DumpStatistics() const noexcept {
    ...
    Logger::GetInstance().Info("EncoderManager", "Interrupt Count: %u", interrupt_count_.load());
    Logger::GetInstance().Info("EncoderManager", "Interrupt Enabled: %s", interrupt_enabled_ ? "YES" : "NO");
