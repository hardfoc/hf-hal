...
bool ImuManager::CreateExternalBno08xDevice(uint8_t deviceIndex, 
    ...
    BaseI2c* i2c_interface = comm_manager_->GetI2cDevice(i2cDeviceId);
    if (!i2c_interface) {
        Logger::GetInstance().Error("ImuManager", "I2C device %u not available", static_cast<uint8_t>(i2cDeviceId));
        return false;
    }
    ...
    Logger::GetInstance().Info("ImuManager", "External BNO08x device %u created successfully", deviceIndex);
